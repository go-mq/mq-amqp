...
const defaultAMQPTestURI = "amqp://127.0.0.1:5672"

func init() {
	if uri := os.Getenv("AMQP_TEST_URI"); uri != "" {
		amqpURI = uri
	} else {
		amqpURI = defaultAMQPTestURI
	}
...
func TestRunAMQPSuite(t *testing.T) {
...
func TestAMQPHeaders(t *testing.T) {
	broker, err := mq.NewBroker(amqpURI)
