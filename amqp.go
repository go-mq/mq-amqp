...
func init() {
	...
	mq.Register("amqp", func(uri string) (mq.Broker, error) {
		return New(uri)
	})
}
...
// Broker implements the mq.Broker interface for AMQP, such as RabbitMQ.
type Broker struct {
	mut        sync.RWMutex
...
func New(url string) (mq.Broker, error) {
...
func (b *Broker) Queue(name string) (mq.Queue, error) {
	...
	q, err := b.channel().QueueDeclare(
		...
		amqp.Table{
			...
			"x-max-priority":            uint8(mq.PriorityUrgent),
		},
	)
...
func (q *Queue) Publish(j *mq.Job) (err error) {
	if j == nil || j.Size() == 0 {
...
func (i *JobIter) nextWithTimeout(timeout time.Duration) (*mq.Job, error) {
	select {
	case d, ok := <-i.c:
		if !ok {
			return nil, mq.ErrAlreadyClosed.New()
...
func fromDelivery(d *amqp.Delivery) (*mq.Job, error) {
	j := mq.NewJob()

	j.ID = d.MessageId
	j.Priority = mq.Priority(d.Priority)
